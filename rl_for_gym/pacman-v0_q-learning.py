...
def q_learning(agent, n_episodes_lim, n_steps_lim, lr, do_render=False):
    ...
    flattened_obs_space_dim = np.prod(agent.env.observation_space.shape)
    agent.q_values = np.empty((0, flattened_obs_space_dim, agent.env.action_space.n))

    # initialize q-values table
    q_values = np.zeros((flattened_obs_space_dim, agent.env.action_space.n))
    ...
    for ep in np.arange(n_episodes_lim):
        ...
        flattened_state = state.reshape(flattened_obs_space_dim)
        ...
        for k in np.arange(n_steps_lim):
            ...
            if np.random.rand() > epsilon:
                action = np.argmax(q_values[flattened_state])
            ...
            flattened_new_state = new_state.reshape(flattened_obs_space_dim)

            # update q values
            q_values[flattened_state, action] += lr * (
                  r \
                + agent.gamma * np.max(q_values[flattened_new_state, :]) \
                - q_values[flattened_state, action]
            )
            ...
            # update state
            flattened_state = flattened_new_state

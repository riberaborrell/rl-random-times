...
class ReplayMemoryAdvantage(ReplayMemory):
    ''' replay memory for storing state-action pairs, returns, advantages and values
    '''
    ...
    def reset(self):
        ...
        if self.is_action_continuous:
            ...
            self.actions = np.full(self.max_size, np.nan, dtype=np.float32)
        self.returns = np.full(self.max_size, np.nan, dtype=np.float32)
        self.advantages = np.full(self.max_size, np.nan, dtype=np.float32)
        self.values = np.full(self.max_size, np.nan, dtype=np.float32)

        # counters and flags
        self.reset_counters()

    def store(self, state, action, ret, advantage, value):
    ...
    def store_vectorized(self, states, actions, returns, advantages, values):
        n_experiences = states.shape[0]
        i = self.ptr
        j = self.ptr + n_experiences
        assert j < self.max_size, 'The memory size is too low'
        ...
        self.actions[i:j] = actions
        self.returns[i:j] = returns
        self.advantages[i:j] = advantages
        self.values[i:j] = values
